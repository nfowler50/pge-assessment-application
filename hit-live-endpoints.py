...
login_url = f"http://{base_url}/login" if not base_url.startswith(("http://", "https://")) else f"{base_url}/login"
...
if login_response.status_code == 200:
    ...
    predict_url = f"http://{base_url}/predict" if not base_url.startswith(("http://", "https://")) else f"{base_url}/predict"
    predict_params = {
        "input": input_data
    }
